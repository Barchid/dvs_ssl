...
from project.datamodules.dsec import DSEC
...
def main(args):
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        monitor="train_loss_epoch",  # TODO: select the logged metric to monitor the checkpoint saving
        filename=name + "-{epoch:03d}-{train_loss_epoch:.4f}",
        save_top_k=1,
        mode="max",
    )
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            checkpoint_callback,
            EarlyStopping(monitor="train_loss_epoch", mode="min", patience=5),
        ],
    ...
    # write in score
    report = open(f"report_dsec.txt", "a")
    now = datetime.now()
    dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
    report.write(
        f"{dt_string} {mode} {checkpoint_callback.best_model_score} {trans}\n"
    )
    report.flush()
    report.close()
