...
from project.utils.iternorm import IterNorm


class VIbCRegLoss(nn.Module):
    """Loss function of the VIbCReg method for Self-Supervised Learning
    strongly inspired from: https://arxiv.org/pdf/2109.00783.pdf
    """

    def __init__(self, invariance_loss_weight: float = 25., variance_loss_weight: float = 25., covariance_loss_weight: float = 200.):
        ...
        self.iternorm = IterNorm(128, num_groups=64, T=5)
    
    def _l2_norm(self, a, b):
    ...
    def forward(self, Z_a, Z_b):
        ...
        print(Z_a.shape)
        Z_a = self.iternorm(Z_a)
        Z_b = self.iternorm(Z_b)
        print(Z_a.shape)
        # invariance loss
        ...
        # cov_Z_a = (( (Z_a / self).T @ Z_a) / (N - 1)).square()  # DxD
