# from https://github.com/loiccordone/object-detection-with-spiking-neural-networks/blob/main/models/spiking_densenet.py (loic cordone et al)
...
class MultiStepSpikingDenseNet(SpikingDenseNet):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16), 
                 num_init_channels=2, bn_size=4, drop_rate=0, 
                 num_classes=1000, init_weights=True, norm_layer: callable = None,
                 T=None, neuron_fun: callable = None, **kwargs):
        self.T = T
        super().__init__(growth_rate, block_config, num_init_channels, bn_size, drop_rate, num_classes, init_weights, norm_layer, neuron_fun, **kwargs)
        
        self.final_neurons = neuron.MultiStepIFNode(v_threshold=float('inf'), detach_reset=True, surrogate_function=surrogate.ATan())
    ...
    def forward(self, x, classify=True):
        ...
        if classify:
            ...
            x_seq = functional.seq_to_ann_forward(x_seq, self.fc)
            x_seq = self.final_neurons(x_seq)
            return fm_trans1, fm_trans2, self.final_neurons.v_seq[-1]
...
def _multi_step_densenet(
    ...
    **kwargs: Any,
    ...
    model = MultiStepSpikingDenseNet(growth_rate, block_config, num_init_channels, norm_layer=norm_layer, T=T, neuron_fun=multi_step_neuron, **kwargs)
    return model
