...
class PAFActionRecognition(Dataset):
    """PAFBenchmark action recognition dataset <https://github.com/CrystalMiaoshu/PAFBenchmark>. Events have (xypt) ordering.
    ::
    ...
    base_url = "https://tongjieducn-my.sharepoint.com/:f:/g/personal/ziyang_tongji_edu_cn/EtbRTOGDf8tIgXZZK5Vkg6YB76Q2K1JS2a2NqyYiRjRrpA?e=1oCUiC"
    filename = "NONE"
    base_folder = "Action Recognition"
    ...
    def __init__(self, save_to, transform=None, target_transform=None):
        super(PAFActionRecognition, self).__init__(
        ...
        if not self._check_exists():
            print(
                f"""
                WARNING: this dataset is available from OneDrive and must be downloaded manually.
        ...
        for act_dir in os.listdir(file_path):
            ...
            print(act_dir, '->', len(os.listdir(os.path.join(file_path, act_dir))))
    ...
    def __getitem__(self, index):
        ...
        ts, x, y, pol = getDVSeventsDavis(input_path)
        
        events = np.empty(len(ts), dtype=self.dtype)
        events["x"] = x
        events["y"] = y
        events["t"] = ts
        events["p"] = pol
        
        events['y'] = self.sensor_size[1] - 1 - events['y'] # otherwise, tensors are upside down
    ...
    def _check_exists(self):
        ...
        ) and self._folder_contains_at_least_n_files_of_type(100, ".aedat")
